...
export function reconstructWatchStates(entries: readonly unknown[]): Map<string, WatchState> {
  ...
  for (const raw of entries) {
    ...
    if (entry.type !== "custom" || entry.customType !== LIFECYCLE_ENTRY_TYPE || !isLifecycleRecord(entry.data)) continue;
...
function isLifecycleRecord(value: unknown): value is WatchLifecycleRecord {
  ...
  return (
    ...
    (["started", "finish", "interrupt", "close", "cancelled"] as const).includes(record.kind as never)
  );
}
